...
for specie in species_counts.index:
    ...
    df_specie['source'] = 'base'  # Optionnel pour les données de base
    df_oversampled['source'] = 'oversample'
    augmented_frames.append(df_oversampled)

# Marquer les données de base
data_orig['source'] = 'base'

# Créer un jeu de données "augmenté" à partir de data_orig en ajoutant du bruit
augmented_data = data_orig.copy()
feat_cols = data_orig.columns[:-1]  # toutes les colonnes sauf 'source' (ajoutée en dernière position)
augmented_data[feat_cols] = augmented_data[feat_cols] + np.random.normal(0, 0.5, augmented_data[feat_cols].shape)
augmented_data['source'] = 'augmented'

# Pour la visualisation, nous utilisons les 2 premières features
plt.figure(figsize=(10,6))
# Test data
plt.scatter(X_test.iloc[:,0], X_test.iloc[:,1], c='blue', label='Test Data')
# Données de base
plt.scatter(data_orig.iloc[:,0], data_orig.iloc[:,1], c='green', label='Base Training')
# Oversampling (regroupement de tous les oversamples)
oversampled_df = pd.concat(augmented_frames)
plt.scatter(oversampled_df.iloc[:,0], oversampled_df.iloc[:,1], c='red', label='Oversampled')
# Augmentation réalisée
plt.scatter(augmented_data.iloc[:,0], augmented_data.iloc[:,1], c='orange', label='Augmented')
...
# Répartir la donnée de base : 20% pour la validation et 80% pour le training
val_ratio = 0.2
validation_counts = original_counts * val_ratio
training_base = original_counts - validation_counts
training_totals = training_base + oversample_numbers + random_aug_counts

plt.figure(figsize=(8,6))
species = augmented_counts.index
plt.bar(species, validation_counts, color='blue', label='Validation')
plt.bar(species, training_totals, bottom=validation_counts, color='orange', label='Training (Oversampling + Augmentation)')
plt.xlabel("Espèce")
