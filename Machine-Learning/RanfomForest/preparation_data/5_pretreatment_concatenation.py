...
# ================================
# 8. Validation croisée sur le dataset étendu
# ================================
# Création d'un dataset étendu pour l'ensemble complet des données
X_full = data.drop(columns=[target_col])
y_full = data[target_col]
X_full_extended = X_full.copy()
...
for method_name, func in preprocessing_methods.items():
    ...
    if method_name == "Standard Scaling":
        scaler = StandardScaler()
        X_trans = pd.DataFrame(scaler.fit_transform(X_full), index=X_full.index, columns=X_full.columns)
    else:
        X_trans = func(X_full)
    X_trans = X_trans.add_prefix(f"{method_name}_")
    X_full_extended = pd.concat([X_full_extended, X_trans], axis=1)

cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
cv_scores = cross_val_score(RandomForestClassifier(random_state=42), X_full_extended, y_full, cv=cv, scoring='accuracy')
print("\nValidation croisée sur dataset étendu - Accuracy moyenne :", np.mean(cv_scores))
