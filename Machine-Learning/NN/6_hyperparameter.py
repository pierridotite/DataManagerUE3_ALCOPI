...
class NeuralNet(nn.Module):
    def __init__(self, input_dim, hidden_dim1=64, hidden_dim2=32, output_dim=n_classes, dropout_rate=0.0):
...
# Ajout de la fonction de training avec paramètres personnalisés
def train_and_evaluate(input_dim, hidden_dim1, hidden_dim2, learning_rate, num_epochs, batch_size, dropout_rate, weight_decay):
    # Création d'un nouveau modèle avec les hyperparamètres
    model_hp = NeuralNet(input_dim, hidden_dim1=hidden_dim1, hidden_dim2=hidden_dim2, output_dim=n_classes, dropout_rate=dropout_rate)
    model_hp.to(device)
    local_optimizer = optim.Adam(model_hp.parameters(), lr=learning_rate, weight_decay=weight_decay)
    local_criterion = nn.CrossEntropyLoss()
    # Reconstruction du DataLoader avec le batch_size passé en paramètre
    train_dataset_hp = TensorDataset(X_train_tensor, y_train_tensor)
    local_train_loader = DataLoader(train_dataset_hp, batch_size=batch_size, shuffle=True)
    
    # Boucle d'entraînement
    for epoch in range(num_epochs):
        model_hp.train()
        running_loss = 0.0
        for batch_X, batch_y in local_train_loader:
            batch_X, batch_y = batch_X.to(device), batch_y.to(device)
            local_optimizer.zero_grad()
            outputs = model_hp(batch_X)
            loss = local_criterion(outputs, batch_y)
            loss.backward()
            local_optimizer.step()
            running_loss += loss.item() * batch_X.size(0)
        # Optionnel : calcul de la loss par époque (non affiché)
    
    # Evaluation sur le jeu de validation
    model_hp.eval()
    with torch.no_grad():
        outputs_val = model_hp(X_val_tensor.to(device))
        loss_val = local_criterion(outputs_val, y_val_tensor.to(device)).item()
    return loss_val

# Section d'optimisation des hyperparamètres
print("\n=== Optimisation des hyperparamètres étendue ===")
# Grille de recherche sur quelques hyperparamètres
lr_options = [0.001, 0.005]
hidden1_options = [32, 64]
hidden2_options = [16, 32]
dropout_options = [0.0, 0.5]
weight_decay_options = [0, 0.01]
            ...
            for dropout in dropout_options:
                for wd in weight_decay_options:
                    for bs in batch_size_options:
                        val_loss = train_and_evaluate(input_dim, hidden_dim1=h1, hidden_dim2=h2,
                                                      learning_rate=lr, num_epochs=50, batch_size=bs,
                                                      dropout_rate=dropout, weight_decay=wd)
                        print(f"lr: {lr}, hidden1: {h1}, hidden2: {h2}, dropout: {dropout}, weight_decay: {wd}, batch_size: {bs} -> Val Loss: {val_loss:.4f}")
                        if val_loss < best_loss:
                            best_loss = val_loss
                            best_config = {"learning_rate": lr, "hidden_dim1": h1, "hidden_dim2": h2,
                                           "dropout_rate": dropout, "weight_decay": wd, "batch_size": bs}

print(f"\n==> Meilleure configuration: {best_config} avec une Validation Loss de {best_loss:.4f}")
