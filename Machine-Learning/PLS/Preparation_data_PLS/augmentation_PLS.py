...
for params in ParameterGrid(param_grid_aug):
   ...
   # Entraînement du modèle avec cette configuration d'augmentation
    n_components = 10  # nombre de composantes PLS, tu peux l'ajuster ou l'optimiser aussi

    # 1. PLS pour réduction de dimension
    pls = PLSRegression(n_components=n_components)
    pls.fit(X_train_final, pd.get_dummies(y_train_final))  # one-hot encoding pour PLS

    # 2. Transformation des données
    X_train_pls = pls.transform(X_train_final)
    X_val_pls = pls.transform(X_val)

    # 3. Modèle de classification sur les scores PLS
    clf = LogisticRegression(max_iter=1000, random_state=42)
    clf.fit(X_train_pls, y_train_final)

    # 4. Prédiction
    y_pred = clf.predict(X_val_pls)
...
# ================================
# Entraînement final du modèle PLS + Classifieur
# ================================

# Définir le nombre de composantes pour la PLS (à ajuster si besoin)
n_components = 10  # tu peux tuner ce chiffre !

pls = PLSRegression(n_components=n_components)
pls.fit(X_train_final, pd.get_dummies(y_train_final))  # On fait un one-hot pour la PLS

# Transformation des features
X_train_pls = pls.transform(X_train_final)
X_val_pls = pls.transform(X_val)

# Entraîner une régression logistique sur les scores PLS
clf = LogisticRegression(max_iter=1000, random_state=42)
clf.fit(X_train_pls, y_train_final)
...
# Résultats
# ================================
print("\nClassification Report avec la meilleure configuration d'augmentation (PLS+LR) :")
...
# ================================
# Importance des variables (version PLS)
# ================================
# Les poids de la première composante
importances = np.abs(pls.x_weights_[:,0])

plt.figure(figsize=(10,6))
plt.title("Importances des variables (selon PLS)")
